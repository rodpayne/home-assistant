...
logger.debug(
    "===== triggered entity = {2} ({0}) ; triggered state = {1}".format(
        triggeredEntity, triggeredStatus, triggeredFriendlyName
    )
)
...
if (triggeredEntity.find("sensor.") == 0) and (triggeredEntity.find("_location") > 0):
    ...
    logger.debug(
        "setting sensor name = {0}; old state = {3}; new state = {1}; from entity_id = {2}".format(
            sensorName, newStatus, triggeredEntity, oldStatus
        )
    )

    rest_command = (
        "homeseer_" + personName.lower() + "_" + newStatus.lower().replace(" ", "_")
    )
    try:
...
# --------------------------------------------------------------------------------------------------
    ...
    (triggeredEntity.find("device_tracker.") == 0)
    or (triggeredEntity.find("binary_sensor.") == 0)
    or (triggeredEntity.find("sensor.") == 0)
):
    ...
    if "person_name" in triggeredAttributesObject:
        ...
        logger.warning(
            'account_name (or person_name) attribute is missing, trying "{0}"'.format(
                person_name
            )
        )
    ...
    if triggeredTo == "NotSet":
        ...
        saveThisUpdate = True
        logger.debug(
            "entity {0} does not yet exist (normal at startup)".format(sensorName)
        )
        oldStatus = "none"
        oldAttributesObject = {}
    else:
        ...
        if oldStatus == "unknown":
            ...
            if triggeredTo != triggeredFrom:  # did it change zones?
                ...
                if (
                    not ("source" in oldAttributesObject)
                    or not ("reported_state" in oldAttributesObject)
                    or oldAttributesObject["source"] == triggeredEntity
                ):  # same entity as we are following?
                    saveThisUpdate = True  # same tracker as we are following (or this it the first one)
                elif (
                    triggeredStatus == oldAttributesObject["reported_state"]
                ):  # same status as the one we are following?
                    if not ("vertical_accuracy" in oldAttributesObject) or (
                        "vertical_accuracy" in triggeredAttributesObject
                        and (
                            triggeredAttributesObject["vertical_accuracy"] > 0
                            and oldAttributesObject["vertical_accuracy"] == 0
                        )
                    ):  # better choice based on accuracy?
                        saveThisUpdate = True
                        logger.debug("vertical_accuracy is better")
                    if (
                        "gps_accuracy" in triggeredAttributesObject
                        and "gps_accuracy" in oldAttributesObject
                        and triggeredAttributesObject["gps_accuracy"]
                        < oldAttributesObject["gps_accuracy"]
                    ):  # better choice based on accuracy?
                        saveThisUpdate = True
    ...
    if saveThisUpdate == True:
        logger.debug(
            "account_name/personName = {0}; triggeredFrom = {1}; triggeredTo = {2}; source_type = {3}".format(
                personName, triggeredFrom, triggeredTo, sourceType
            )
        )
        ...
        if "source_type" in triggeredAttributesObject:
            newAttributesObject["source_type"] = triggeredAttributesObject[
                "source_type"
            ]
        else:
        ...
        if "gps_accuracy" in triggeredAttributesObject:
            newAttributesObject["gps_accuracy"] = triggeredAttributesObject[
                "gps_accuracy"
            ]
        else:
        ...
        if "altitude" in triggeredAttributesObject:
            newAttributesObject["altitude"] = round(
                triggeredAttributesObject["altitude"], 0
            )
        if "vertical_accuracy" in triggeredAttributesObject:
            newAttributesObject["vertical_accuracy"] = triggeredAttributesObject[
                "vertical_accuracy"
            ]
        else:
        ...
        if (
            triggeredStatus == "Away"
            or triggeredStatus == "Stationary"
            or triggeredStatus.lower() == "on"
        ):
            friendly_name = "{0} ({1}) is {2}".format(
                string.capwords(personName), triggeredFriendlyName, triggeredStatus
            )
            template = "{0} ({1}) is in <locality>".format(
                string.capwords(personName), triggeredFriendlyName
            )
        else:
            friendly_name = "{0} ({1}) is at {2}".format(
                string.capwords(personName), triggeredFriendlyName, triggeredStatus
            )
        ...
        if "zone" in triggeredAttributesObject:
            ...
            zoneEntityID = "zone." + triggeredStatus.lower().replace(" ", "_").replace(
                "'", "_"
            )
        ...
        if triggeredStatusHomeAway == "Home":
            if (
                ...
                or oldStatus == "just left"
            ):
                newStatus = "Home"
                try:
                    hass.services.call(
                        "rest_command", "homeseer_" + personName.lower() + "_home"
                    )
                except:
                    logger.debug(
                        "rest_command homeseer_{0}_home not defined".format(
                            personName.lower()
                        )
                    )
            elif oldStatus == "home":
                ...
                try:
                    hass.services.call(
                        "rest_command",
                        "homeseer_" + personName.lower() + "_just_arrived",
                    )
                except:
                    logger.debug(
                        "rest_command homeseer_{0}_just_arrived not defined".format(
                            personName.lower()
                        )
                    )
        else:
            if oldStatus == "none" or ha_just_started == "on":
                ...
                try:
                    hass.services.call(
                        "rest_command", "homeseer_" + personName.lower() + "_away"
                    )
                except:
                    logger.debug(
                        "rest_command homeseer_{0}_away not defined".format(
                            personName.lower()
                        )
                    )
                ...
                try:
                    hass.services.call(
                        "rest_command", "homeseer_" + personName.lower() + "_just_left"
                    )
                except:
                    logger.debug(
                        "rest_command homeseer_{0}_just_left not defined".format(
                            personName.lower()
                        )
                    )
            else:
        ...
        logger.debug(
            "setting sensor name = {0}; oldStatus = {1}; newStatus = {2}".format(
                sensorName, oldStatus, newStatus
            )
        )
        ...
        if newStatus != "Home" or ha_just_started == "on":
            try:
                ...
                logger.debug(
                    "person_location reverse_geocode service call exception: {0}".format(
                        str(e)
                    )
                )
