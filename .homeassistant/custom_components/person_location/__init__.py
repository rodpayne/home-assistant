...
class PERSON_LOCATION_INTEGRATION:
    def __init__(self, name, hass, config):
        ...
        self.attributes = {}
        home_zone = 'zone.home'
        self.attributes['friendly_name'] = f"{INTEGRATION_NAME} Service"
        self.attributes['home_latitude'] = str(hass.states.get(home_zone).attributes.get(ATTR_LATITUDE))
        self.attributes['home_longitude'] = str(hass.states.get(home_zone).attributes.get(ATTR_LONGITUDE))
        ...
        self.configured_osm_api_key = config[DOMAIN].get(CONF_OSM_API_KEY,CONF_API_KEY_NOT_SET)
        self.configured_google_api_key = config[DOMAIN].get(CONF_GOOGLE_API_KEY,CONF_API_KEY_NOT_SET)
        self.configured_language = config[DOMAIN].get(CONF_LANGUAGE,DEFAULT_LANGUAGE)
        self.configured_region = config[DOMAIN].get(CONF_REGION,DEFAULT_REGION)
...
def setup(hass, config):
    ...
    def handle_reverse_geocode(call):
        ...
        try:
                ...
                #                        _LOGGER.debug( '(' + entity_id + ') location_type = ' + location_type + '; formatted_address = ' + formatted_address)
                        ...
                        if distance_from_home <= 0:
                            ...
                            try:
                                ...
                                wazeErrorCount = pli.attributes['waze_error_count'] + 1
                                pli.attributes['waze_error_count'] = wazeErrorCount
                                hass.states.set(pli.name, pli.state, pli.attributes)
            ...
            _LOGGER.debug(traceback.format_exc())
            apiErrorCount = pli.attributes['api_error_count'] + 1
            pli.attributes['api_error_count'] = apiErrorCount
            hass.states.set(pli.name, pli.state, pli.attributes)
