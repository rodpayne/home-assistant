...
def setup(hass, config):
    ...
    configured_google_api_key = config[DOMAIN].get(
    ...
    def handle_reverse_geocode(call):
        ...
        if apiStateObject != None:
            ...
            try:
                if "api_calls_skipped" in apiAttributesObject:
                    skippedApiCalls = apiAttributesObject["api_calls_skipped"]
                else:
                    skippedApiCalls = 0
                if "api_last_updated" in apiAttributesObject:
                    lastApiTime = apiAttributesObject["api_last_updated"]
                else:
                    lastApiTime = currentApiTime - THROTTLE_INTERVAL
                if "api_calls_attempted" in apiAttributesObject:
                    attemptedApiCalls = apiAttributesObject["api_calls_attempted"]
                else:
                    attemptedApiCalls = 0
                ...
                if apiStatus != "on":
                    skippedApiCalls += 1
                    apiAttributesObject["api_calls_skipped"] = skippedApiCalls
                    ...
                    if wait_time > 0:
                        _LOGGER.debug(
                            "(" + entity_id + ") wait_time = " + str(wait_time)
                        )
                    ...
                    apiAttributesObject["api_last_updated"] = currentApiTime

                    apiAttributesObject["api_calls_attempted"] = attemptedApiCalls + 1
                        ...
                        if new_latitude == "None" or new_longitude == "None":
                            ...
                            if "update_time" in targetAttributesObject:
                                new_update_time = datetime.strptime(
                                    targetAttributesObject["update_time"],
                                    "%Y-%m-%d %H:%M:%S.%f",
                                )
                                _LOGGER.debug(
                                    "("
                            ...
                            if "previous_update_time" in targetAttributesObject:
                                old_update_time = datetime.strptime(
                                    targetAttributesObject["previous_update_time"],
                                    "%Y-%m-%d %H:%M:%S.%f",
                                )
                                _LOGGER.debug(
                                    "("
                            ...
                            if "meters_from_home" in targetAttributesObject:
                                old_distance_from_home = float(
                                    targetAttributesObject["meters_from_home"]
                                )

                            if (
                                "reported_state" in targetAttributesObject
                                and targetAttributesObject["reported_state"].lower()
                                == "home"
                            ):
                            ...
                            targetAttributesObject["meters_from_home"] = str(
                                round(distance_from_home, 1)
                            )
                            targetAttributesObject["miles_from_home"] = str(
                            ...
                            if configured_osm_api_key != CONF_API_KEY_NOT_SET:
                                ...
                                targetAttributesObject[
                                    "friendly_name"
                                ] = template.replace("<locality>", locality)
                                targetAttributesObject[
                                    "OSM_location"
                            ...
                            if configured_google_api_key != CONF_API_KEY_NOT_SET:
                                ...
                                if configured_google_api_key == CONF_API_KEY_NOT_SET:
                                    google_url = (
                                        "https://maps.google.com/maps/api/geocode/json?language=en&region=US&latlng="
                                        + str(new_latitude)
                                        + ","
                                        + str(new_longitude)
                                    )
                                    ...
                                    google_url = (
                                        "https://maps.googleapis.com/maps/api/geocode/json?language=en&region=US&latlng="
                                        + str(new_latitude)
                                        + ","
                                        + str(new_longitude)
                                        + "&key="
                                        + configured_google_api_key
                                    )
                                google_decoded = {}
                            ...
                            targetAttributesObject["previous_update_time"] = str(
                                new_update_time
                            )
                            ...
                            if distance_from_home == 0:
                                ...
                                to_address = (
                                    str(home_latitude) + "," + str(home_longitude)
                                )
                                region = "US"
                                try:
                                    ...
                                    if routeDistance >= 100:
                                        targetAttributesObject["driving_miles"] = str(
                                    ...
                                    targetAttributesObject["driving_minutes"] = str(
                                        round(routeTime, 1)
                                    )
                                    attribution += (
                                    ...
                                    wazeErrorCount = (
                                        apiAttributesObject["waze_error_count"] + 1
                                    )
                                    apiAttributesObject[
                                        "waze_error_count"
                                    ] = wazeErrorCount
                                    hass.states.set(
                                        API_STATE_OBJECT, apiStatus, apiAttributesObject
                                    )
    ...
    apiAttributesObject["api_last_updated"] = datetime.now()
    apiAttributesObject["api_calls_attempted"] = 0
